...
### 交易成本（Transaction Cost）：开源的“胜利之匙”
...
* 开源的本质，是互联网和 Git 把协作的交易成本（Transaction Cost of Collaboration） 降到了低于企业边界的水平。
  ...
  * Linux 能够和 Windows 一样为现实世界发挥力量，不是因为 Linus 更聪明，而是因为 Linux 社区的**“管理成本”接近于零**，而微软为了协调几千名开发者，内部消耗了巨大的管理成本（文山会海、流程审批）。
...
* 开源通过 License 创造了一种**“新型产权”**。
  它既不是完全的公有（谁都能破坏），也不是完全的私有（谁都不能看）。它是**“共有产权”（Common Property）**。
...
### 制度变迁与路径依赖（Institutional Change & Path Dependence）
...
这就是为什么会出现**“货船崇拜”（Cargos Cult）：模仿了制度的外形**（基金会、代码库），但无法复制制度的非正式约束（信任、平等、黑客伦理）。
